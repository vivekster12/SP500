...
```{r}
temp <- array(minmax_scaler(sp500_training_complete$Close,0,1))
array(inverse_minmax_scaler(temp))
sp500_training_complete$Close
```
...
```{r}
feats = array(feats, dim=c(total, N,1))
```
...
```{r}
sp500_total$Close[(dim(sp500_total)[1] - total-N):dim(sp500_total)[1]]
```


```{r}
sp500_total = rbind(sp500_training_complete,sp500_training_complete)
sp500_testing_scaled = array(minmax_scaler(sp500_total$Close[(dim(sp500_total)[1] - total-N):dim(sp500_total)[1]],0,1))
...
{
  s = i-1+N
  test_feats = rbind(feats,modified_sp500_testing_scaled[i:s])
}

test_feats = array(test_feats, dim=c(total+N, N,1))
```
